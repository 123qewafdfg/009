...
var WEBP_CONVERTIBLE_EXTENSIONS = ['jpg', 'jpeg', 'png', 'bmp', 'tiff', 'svg', 'ico', 'heic'];
...
function canConvertFileToWebP(file) {
    if (!file || !file.name) return false;
    return WEBP_CONVERTIBLE_EXTENSIONS.indexOf(getLowercaseFileExtension(file.name)) !== -1;
}
...
function getLZString() {
    if (typeof window !== 'undefined') {
        return window.LZString || window.lzstring || null;
    }
...
function compressLargeData(data, chunkSize = 1024 * 1024) {
    var lzString = getLZString();
    if (!lzString) {
        throw new Error('LZString 未加载');
    }
...
function loadFileAsURL(){
    ...
    if (!isTailMode && totalSize > LARGE_TAIL_FILE_THRESHOLD) {
        hideProgressBar();
        imageMsg.textContent = '大文件请切换到“文件尾追加”模式。PNG/JPG 像素隐写不适合处理 ' + formatFileSize(totalSize) + ' 的文件。';
        mainFile.value = '';
        return;
    }
	...
	function processFileAsync(index) {
		...
		setTimeout(function() {
			...
			if (isTailMode && fileToLoad.type.slice(0,4) != "text") {
                if (!fileToLoad.type.startsWith('image/') || fileToLoad.size >= LARGE_TAIL_FILE_THRESHOLD) {
                    binaryFilesForTail.push({
                        name: fileToLoad.name,
                        type: fileToLoad.type || 'application/octet-stream',
                        data: fileToLoad,
                        useBlob: true
                    });
                    actualTotalSize += fileToLoad.size;
                    createTailFilePreview(fileToLoad);
				...
				previewReader.onload = function(e) {
					...
					var isConvertibleToWebP = canConvertFileToWebP(fileToLoad);
					console.log('文件类型:', fileToLoad.type, '允许转WebP:', isConvertibleToWebP, 'WebP开关:', webpEnabled);
					
					if(isConvertibleToWebP && webpEnabled) {
						...
						appendManagedImagePreview(imageListBox, {
							...
							previewSource: fileToLoad
								...
								var webpFilename = getWebPFilename(fileToLoad.name);
				...
				fileReader.onload = function(fileLoadedEvent){
					...
					if(fileToLoad.type.slice(0,4) == "text"){
						...
						var isConvertibleToWebP2 = canConvertFileToWebP(fileToLoad);
						console.log('[非尾追加模式] 文件类型:', fileToLoad.type, '允许转WebP:', isConvertibleToWebP2, 'WebP开关:', webpEnabled2);
						
						if(isConvertibleToWebP2 && webpEnabled2) {
							var qualityReduction = getQualityReduction();
							...
							convertToWebP(fileToLoad, qualityReduction, function(error, webpDataUrl, info) {
								...
								if(error) {
									...
									var div = document.createElement('div');
									div.className = 'file-list-item';
									var webpFilename = getWebPFilename(fileName);
...
function loadImage(){
	...
	function processTailModeImage(fileToLoad, next) {
		...
		if (webpEnabled && canConvertFileToWebP(fileToLoad)) {
			var qualityReduction = getQualityReduction();
...
var LARGE_TAIL_FILE_THRESHOLD = 20 * 1024 * 1024;

function getTailFileDataSize(fileData) {
    if (!fileData || !fileData.data) return 0;
    if (typeof fileData.data.byteLength === 'number') return fileData.data.byteLength;
    if (typeof fileData.data.size === 'number') return fileData.data.size;
    return 0;
}

function shouldUseBlobTailPipeline(totalSize) {
    if (totalSize >= LARGE_TAIL_FILE_THRESHOLD) return true;
    for (var i = 0; i < binaryFilesForTail.length; i++) {
        if (binaryFilesForTail[i] && binaryFilesForTail[i].useBlob) return true;
    }
    return false;
}

function createTailFilePreview(file) {
    var div = document.createElement('div');
    div.className = 'file-list-item';
    var link = document.createElement('a');
...
function appendBlobDataToCover(coverSource, contentType, dataParts, filenameExt) {
    var parts = [coverSource].concat(dataParts);
    var newBlob = new Blob(parts, { type: contentType || 'image/png' });
    var newUrl = URL.createObjectURL(newBlob);

    if (!isLikelyMobileDevice() && newBlob.size < LARGE_TAIL_FILE_THRESHOLD) {
        preview.src = newUrl;
    }
    processedImageData = newUrl;
    processedImageFilename = 'stego-image-tail.' + (filenameExt || ((contentType || '').indexOf('png') !== -1 ? 'png' : 'jpg'));
    enableDownload();
    hideProgressBar();
    if (isLikelyMobileDevice() || newBlob.size >= LARGE_TAIL_FILE_THRESHOLD) {
        imageMsg.textContent = '大文件已追加完成。为避免手机卡死，结果不会重新预览，请直接点击下载保存。';
    } else {
        imageMsg.textContent = '数据已追加到图片文件尾。点击下载按钮保存图片。';
    }
...
function appendToFileTail(){    
    ...
    setTimeout(function() {
        try {
            ...
            if(hasBinaryFiles) {
                ...
                binaryFilesForTail.forEach(function(fileData, index) {
                    ...
                    metadata.files.push({
                        ...
                        size: getTailFileDataSize(fileData)
                    });
                    
                    // 添加到二进制数据
                    binaryParts.push(fileData.useBlob ? fileData.data : new Uint8Array(fileData.data));
                    totalSize += getTailFileDataSize(fileData);
                });
            }
            ...
            if (shouldUseBlobTailPipeline(totalSize)) {
                updateProgress(55, '正在使用大文件安全模式...');
                metadata.compressed = false;
                var metadataJsonForBlob = JSON.stringify(metadata);
                var metadataBytesForBlob = new TextEncoder().encode(metadataJsonForBlob);
                var magicTextForBlob = 'PASSLOK_BINARY_TAIL';
                var magicBytesForBlob = new TextEncoder().encode(magicTextForBlob);
                var metadataLengthBytes = new Uint8Array(4);
                metadataLengthBytes[0] = metadataBytesForBlob.length & 0xFF;
                metadataLengthBytes[1] = (metadataBytesForBlob.length >> 8) & 0xFF;
                metadataLengthBytes[2] = (metadataBytesForBlob.length >> 16) & 0xFF;
                metadataLengthBytes[3] = (metadataBytesForBlob.length >> 24) & 0xFF;
                var dataPartsForBlob = [magicBytesForBlob, metadataLengthBytes, metadataBytesForBlob];
                for (var blobPartIndex = 0; blobPartIndex < binaryParts.length; blobPartIndex++) {
                    dataPartsForBlob.push(binaryParts[blobPartIndex]);
                }
                dataPartsForBlob.push(magicBytesForBlob);
                isEncodingInProgress = true;
                var appendLargePayload = function(coverSource, contentType) {
                    updateProgress(90, '正在追加大文件...');
                    appendBlobDataToCover(coverSource, contentType, dataPartsForBlob);
                };
                if (originalCoverFile) {
                    appendLargePayload(originalCoverFile, originalCoverFile.type || 'image/png');
                } else {
                    fetch(preview.src)
                        .then(function(response) {
                            var contentType = response.headers.get('content-type') || 'image/png';
                            return response.blob().then(function(blob) {
                                appendLargePayload(blob, contentType);
                            });
                        })
                        .catch(function(error) {
                            hideProgressBar();
                            isEncodingInProgress = false;
                            imageMsg.textContent = '追加大文件时出错：' + error.message;
                        });
                }
                return;
            }
...
// 处理提取的内容，分离图片和文本
function bindPendingExtractedBlobLinks(containerElement) {
    if (!containerElement || !pendingExtractedBlobFiles || pendingExtractedBlobFiles.length === 0) return;
    var placeholders = containerElement.querySelectorAll('.extracted-blob-placeholder[data-blob-index]');
    placeholders.forEach(function(placeholder) {
        var index = parseInt(placeholder.getAttribute('data-blob-index'), 10);
        var fileInfo = pendingExtractedBlobFiles[index];
        if (!fileInfo || !fileInfo.blob) return;

        var link = document.createElement('a');
        link.download = fileInfo.name;
        link.href = URL.createObjectURL(fileInfo.blob);
        link.textContent = fileInfo.name + ' (' + formatFileSize(fileInfo.blob.size) + ')';
        link.setAttribute('contenteditable', 'false');
        ...
        fileInfo.data = link.href;
        fileInfo.element = link;
...
function processExtractedContent(content, isDecodeScreen) {
	...
	if(isDecodeScreen) {
		...
		bindPendingExtractedBlobLinks(boxElement);
		// 给所有链接添加不可编辑属性
...
function collectExtractedFiles(containerElement) {
	var fileLinks = containerElement.querySelectorAll('a[download][href^="data:"], a[download][href^="blob:"]');
	var images = containerElement.querySelectorAll('img[src^="data:"]');
	
	fileLinks.forEach(function(link) {
		extractedFiles.push({
			type: link.getAttribute('href').indexOf('blob:') === 0 ? 'blob-link' : 'file',
...
function decode(){  
    ...
    if (previewElement && previewElement._sourceFile && previewElement._sourceFile.size >= LARGE_TAIL_FILE_THRESHOLD) {
        checkLargeFileTailStego(previewElement, pwdArray[0], function(tailData, tailMsg) {
            if (tailMsg && tailMsg.slice(0,6) == 'Reveal') {
                var fileName = document.getElementById('previewFilename').textContent || '大文件';
                var wrappedContent = '<div class="extracted-result-block"><div class="extracted-image-title-row"><div class="extracted-image-title" contenteditable="false" spellcheck="false">' + fileName + ' - 提取结果</div></div><div class="extracted-result-content">' + tailData + '</div></div>';
                processExtractedContent(wrappedContent, isDecodeScreen);
                if(isDecodeScreen) { extractedContent = wrappedContent; enableDecodeDownload(); }
                msgElement.textContent = '大文件提取成功，请点击提取出的下载链接保存。';
                pwdElement.value = '';
                return;
            }
            msgElement.textContent = tailMsg || '未找到文件尾隐写数据';
        });
        return;
    }
...
function decodeMultipleImages(images, msgElement, pwdElement, boxElement, isDecodeScreen) {
	...
	function processSingleImage(imageData, index) {
        if (imageData.file && imageData.file.size >= LARGE_TAIL_FILE_THRESHOLD) {
            var tailSource = {
                _sourceFile: imageData.file,
                _sourceBlob: imageData.file,
                src: imageData.dataUrl
            };
            checkLargeFileTailStego(tailSource, password, function(tailData, tailMsg) {
                if (tailMsg && tailMsg.slice(0,6) == 'Reveal') {
		...
		tempImg._sourceFile = imageData.file;
		tempImg._sourceBlob = imageData.file;
		tempImg.src = imageData.dataUrl;

		tempImg.onload = function() {
			// 先检查文件尾隐写
            if (imageData.file && imageData.file.size >= LARGE_TAIL_FILE_THRESHOLD) {
                checkLargeFileTailStego(tempImg, password, function(tailData, tailMsg) {
                    if (tailMsg && tailMsg.slice(0,6) == 'Reveal') {
                        extractedDataList.push({
                            imageName: imageData.name,
                            content: tailData
...
function bytesEqualAt(buffer, pattern, index) {
    if (index < 0 || index + pattern.length > buffer.length) return false;
...
function findBytes(buffer, pattern, fromIndex) {
    var start = Math.max(0, fromIndex || 0);
    for (var i = start; i <= buffer.length - pattern.length; i++) {
        if (bytesEqualAt(buffer, pattern, i)) return i;
    }
    return -1;
}

function findBytesReverse(buffer, pattern, fromIndex) {
    var start = Math.min(fromIndex == null ? buffer.length - pattern.length : fromIndex, buffer.length - pattern.length);
...
function getSourceBlobFromImageElement(imageElement) {
    if (imageElement && imageElement._sourceFile) return Promise.resolve(imageElement._sourceFile);
    if (imageElement && imageElement._sourceBlob) return Promise.resolve(imageElement._sourceBlob);
    return fetch(imageElement.src).then(function(response) { return response.blob(); });
}

function createExtractedDownloadLink(file, blob, index) {
    var itemIndex = pendingExtractedBlobFiles.length;
    var fileName = file.name || ('extracted_file_' + (index + 1));
    pendingExtractedBlobFiles.push({
        type: 'blob',
        name: fileName,
        blob: blob,
        data: '',
    ...
    var div = document.createElement('div');
    div.className = 'file-list-item extracted-blob-placeholder';
    div.setAttribute('data-blob-index', String(itemIndex));
    div.setAttribute('data-file-name', fileName);
    div.textContent = fileName + ' (' + formatFileSize(blob.size) + ')';
    return div.outerHTML;
}

function checkLargeFileTailStego(imageElement, password, callback) {
    getSourceBlobFromImageElement(imageElement)
        .then(function(blob) {
            var MAGIC = 'PASSLOK_BINARY_TAIL';
            var magicBytes = new TextEncoder().encode(MAGIC);
            var tailSize = Math.min(blob.size, 1024 * 1024);
            return blob.slice(blob.size - tailSize).arrayBuffer().then(function(tailBuffer) {
                var tail = new Uint8Array(tailBuffer);
                var endInTail = findBytesReverse(tail, magicBytes);
                if (endInTail === -1) {
                    callback('', '没有找到文件尾隐写数据');
                    return;
                }
                var endAbsolute = blob.size - tailSize + endInTail;
                var headerSize = Math.min(blob.size, 2 * 1024 * 1024);
                return blob.slice(0, headerSize).arrayBuffer().then(function(headerBuffer) {
                    var header = new Uint8Array(headerBuffer);
                    var startIndex = findBytes(header, magicBytes, 0);
                    if (startIndex === -1) {
                        callback('', '没有找到文件尾隐写数据');
                        return;
                    }
                    ...
                    var metadataLength = header[pos] | (header[pos + 1] << 8) | (header[pos + 2] << 16) | (header[pos + 3] << 24);
                    pos += 4;
                    if (metadataLength < 0 || metadataLength > 1024 * 1024 || pos + metadataLength > header.length) {
                        callback('', '元数据损坏');
                        return;
                    }
                    var metadataJson = new TextDecoder().decode(header.subarray(pos, pos + metadataLength));
                    ...
                    if (metadata.compressed) {
                        callback('', '该文件使用了压缩格式，请在电脑端提取');
                        return;
                    }
                    var payloadStart = startIndex + magicBytes.length + 4 + metadataLength;
                    ...
                    for (var i = 0; i < metadata.files.length; i++) {
                        ...
                        var fileEnd = offset + file.size;
                        if (fileEnd > endAbsolute) {
                            callback('', '隐藏文件数据不完整');
                            return;
                        }
                        var fileBlob = blob.slice(fileStart, fileEnd, file.type || 'application/octet-stream');
                        if (file.type === 'text/html' && fileBlob.size < 2 * 1024 * 1024) {
                            resultHtml += '<div class="file-list-item">已提取文本内容，请点击下载保存。</div>';
                            resultHtml += createExtractedDownloadLink(file, fileBlob, i);
                        } else {
                            resultHtml += createExtractedDownloadLink(file, fileBlob, i);
                        }
                        offset = fileEnd;
        ...
        .catch(function(error) {
            console.error('Large tail extraction error:', error);
            callback('', '提取大文件时出错：' + error.message);
        });
}
...
// 打开下载弹窗
function syncExtractedFilesFromRenderedContent() {
    var containers = [];
    if (typeof mainBoxDecode !== 'undefined' && mainBoxDecode) containers.push(mainBoxDecode);
    if (typeof imageListBoxDecode !== 'undefined' && imageListBoxDecode) containers.push(imageListBoxDecode);

    containers.forEach(function(container) {
        var links = container.querySelectorAll('a[download][href]');
        links.forEach(function(link) {
            var href = link.getAttribute('href') || '';
            var name = link.getAttribute('download') || (link.textContent || 'downloaded_file').trim() || 'downloaded_file';
            var exists = extractedFiles.some(function(file) {
                return file && file.name === name && (file.data === href || (file.element && file.element === link));
            });
            if (exists) return;

            var pendingMatch = null;
            if (pendingExtractedBlobFiles && pendingExtractedBlobFiles.length) {
                pendingMatch = pendingExtractedBlobFiles.find(function(file) {
                    return file && file.name === name;
                });
            }

            if (pendingMatch && pendingMatch.blob) {
                pendingMatch.data = href;
                pendingMatch.element = link;
                extractedFiles.push(pendingMatch);
                return;
            }

            extractedFiles.push({
                type: href.indexOf('blob:') === 0 ? 'blob-link' : 'file',
                name: name,
                data: href,
                element: link
            });
...
function openDownloadModal() {
	syncExtractedFilesFromRenderedContent();
...
function downloadSingleFile(fileInfo) {
	var link = document.createElement('a');
	link.href = fileInfo.blob ? URL.createObjectURL(fileInfo.blob) : fileInfo.data;
	link.download = fileInfo.name;
	document.body.appendChild(link);
	link.click();
	document.body.removeChild(link);
...
function packageDownload() {
	...
	// 处理提取的文件
	extractedFiles.forEach(function(file) {
		// 优先使用 blob 对象（从 pendingExtractedBlobFiles 来的）
		if (file.blob) {
			zip.file(file.name, file.blob);
			return;
		}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function calculateHiddenFileSize() {
    ...
    binaryFilesForTail.forEach(file => {
      if (file.data) {
        totalSize += getTailFileDataSize(file);
      }
    });
